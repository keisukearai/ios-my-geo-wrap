...
final class WallpaperRecorder: ObservableObject {
    ...
    func startAnimal(animalKind: AnimalKind, speed: Double, colorHue: Double,
                     stillSnapshot: CGImage? = nil) async {
        let screenSize = windowSize()
        let authStatus = await PHPhotoLibrary.requestAuthorization(for: .addOnly)
        guard authStatus == .authorized || authStatus == .limited else {
        ...
        state = .rendering(0)
        do {
            let (videoURL, still, contentID) = try await renderAnimalVideo(
                animalKind: animalKind, speed: speed, colorHue: colorHue,
    ...
    // MARK: - Render Video (ANIMAL)

    private func renderAnimalVideo(
        animalKind: AnimalKind, speed: Double, colorHue: Double,
        ...
        let videoURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("animal_\(UUID().uuidString).mov")
        ...
        // Pre-compute targets once (SpriteKit space)
        let targets = animalKind.buildTargets(count: 220, seed: 42, in: screenSize)
        let frameView = AnimalFrame(t: 0, speed: speed, colorHue: colorHue,
                                    targets: targets, size: screenSize)
    ...
    // MARK: - Start (HOURGLASS)

    func startHourglass(speed: Double, colorHue: Double, form: Double = 0.5,
                        stillSnapshot: CGImage? = nil) async {
        let screenSize = windowSize()
        let authStatus = await PHPhotoLibrary.requestAuthorization(for: .addOnly)
        guard authStatus == .authorized || authStatus == .limited else {
        ...
        state = .rendering(0)
        do {
            let (videoURL, still, contentID) = try await renderHourglassVideo(
                speed: speed, colorHue: colorHue, form: form,
                screenSize: screenSize, stillSnapshot: stillSnapshot
            )
            state = .saving
            try await saveLivePhoto(videoURL: videoURL, stillImage: still, contentIdentifier: contentID)
    ...
    // MARK: - Render Video (HOURGLASS)

    private func renderHourglassVideo(
        speed: Double, colorHue: Double, form: Double = 0.5,
        screenSize: CGSize, stillSnapshot: CGImage? = nil
    ) async throws -> (URL, CGImage, String) {
        let totalFrames = Int(duration) * fps
        let renderScale: CGFloat = 2.0
        ...
        let videoURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("hourglass_\(UUID().uuidString).mov")
        ...
        let frameView = HourglassFrame(t: 0, speed: speed, colorHue: colorHue, form: form, size: screenSize)
        let renderer  = ImageRenderer(content: frameView)
        renderer.scale = renderScale
    ...
    func startFlower(kind: FlowerKind, speed: Double, colorHue: Double, startT: Double = 0) async {
        let screenSize = windowSize()
        let authStatus = await PHPhotoLibrary.requestAuthorization(for: .addOnly)
        guard authStatus == .authorized || authStatus == .limited else {
            state = .failed("Photos access denied\nGo to Settings > Privacy > Photos")
        ...
        state = .rendering(0)
        do {
            let (videoURL, still, contentID) = try await renderFlowerVideo(
                kind: kind, speed: speed, colorHue: colorHue,
                screenSize: screenSize, startT: startT
            )
            state = .saving
            try await saveLivePhoto(videoURL: videoURL, stillImage: still, contentIdentifier: contentID)
            state = .done
    ...
    // MARK: - Render Video (FLOWER)

    private func renderFlowerVideo(
        kind: FlowerKind, speed: Double, colorHue: Double,
        screenSize: CGSize, startT: Double
    ) async throws -> (URL, CGImage, String) {
        let totalFrames = Int(duration) * fps
        let renderScale: CGFloat = 2.0
        let videoSize   = CGSize(width:  screenSize.width  * renderScale,
        ...
        let videoURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("flower_\(UUID().uuidString).mov")
        ...
        let frameView = FlowerFrame(t: startT, kind: kind, speed: speed,
                                    colorHue: colorHue, size: screenSize)
        let renderer  = ImageRenderer(content: frameView)
        renderer.scale = renderScale
